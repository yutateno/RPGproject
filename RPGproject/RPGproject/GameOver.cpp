...
void GameOver::Draw_Start() {
	DrawStringToHandle(0, 0, "ゲームオーバー画面", WHITE, Font::Get(eFont::SELECT));
	DrawFormatStringToHandle(0, 100, WHITE, Font::Get(eFont::SELECT), "開始画面%d", this->startCount);
}
void GameOver::Draw_Main() {
	DrawStringToHandle(0, 0, "ゲームオーバー画面", WHITE, Font::Get(eFont::SELECT));
